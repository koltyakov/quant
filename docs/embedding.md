...
|---|---|---|
| Ollama | `ollama` | Local embedding via `/api/embed`. Auto-detected for loopback URLs (`localhost`, `127.0.0.1`, and `::1`). |
...
OpenAI hostnames are auto-detected as `openai`. For any other remote or compatible service, set `--embed-provider` explicitly. The config fields are named generically (`embed_url`, `embed_model`) so the same configuration surface works for both backends.
...
3. **Still unavailable** — if local Ollama remains unavailable or its model cannot be pulled, `quant` starts in keyword-only mode. The MCP server remains operational and search uses FTS5 keyword results. Other provider, authentication, URL, and protocol errors may remain fatal and must be corrected before startup.
...
2. Chunks are sent to the configured backend in batches of `--embed-batch-size` (default 16).
...
1. The search query is embedded via the `CachingEmbedder`, which wraps the configured backend with:
...
Embedding services have finite input limits. `quant` truncates embedding input to fit within 4000 runes, preferring natural boundaries (paragraphs, sentences, words) over hard cuts.
